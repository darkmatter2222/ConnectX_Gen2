"""Full 10-game MCTS vs AB comparison at 8x7/5."""
...
if __name__ == '__main__':
    print("=== 8x7/5: MCTS (500 sims) vs AB (depth 8) — 10 Game Comparison ===")
    print()
    ...
    print(f"AB as P1: {ab_wins_p1}W | AB as P2: {ab_wins_p2}W")
    print(f"MCTS as P1: {mcts_wins_p1}W | MCTS as P2: {mcts_wins_p2}W")
    print(f"Draws: {draws}")
    print(f"Total time: {total_time:.0f}s")

    # AB total wins
    ab_total = ab_wins_p1 + ab_wins_p2
    mcts_total = mcts_wins_p1 + mcts_wins_p2
    print()
    if draws > 0:
        print(f"AB: {ab_total} wins out of {ab_total+draws} decisive games ({ab_total/(ab_total+draws):.0%})")
        print(f"MCTS: {mcts_total} wins out of {mcts_total+draws} decisive games ({mcts_total/(mcts_total+draws):.0%})")
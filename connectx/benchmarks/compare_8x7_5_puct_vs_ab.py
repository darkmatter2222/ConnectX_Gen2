"""Compare PUCT MCTS vs AB at 8x7/5."""
...
from connectx.bots.mcts_8x7_5_puct import mcts_puct_bot_fast_8x7_5 as MCTS_PUCT


def run_comparison(name, bot1_func, bot2_func, n_games, mcts_sims=2500):
    b1_wins = 0
    b2_wins = 0
    draws = 0
    ...
    for i in range(n_games):
        ...
        result, n, t = play_game(b1m, bot1_func, b2m, bot2_func, mcts_sims)
...
def play_game(b1_mark, b1_func, b2_mark, b2_func, mcts_sims=2500):
...
if __name__ == '__main__':
    print("=== 8x7/5: PUCT MCTS vs AB ===\n")

    print("--- PUCT (fast, 2500 iters) as P1(mark=1) vs AB as P2(mark=2) ---")
    r1 = run_comparison('PUCT_2500', MCTS_PUCT, AB, 10, mcts_sims=2500)

    print("--- AB as P1(mark=1) vs PUCT (fast, 2500 iters) as P2(mark=2) ---")
    r2 = run_comparison('AB', AB, MCTS_PUCT, 10, mcts_sims=2500)
    ...
    print(f"  PUCT: {mcts_total}, AB: {ab_total}, Draw: {draw_total}/{20}" +
          (f" | PUCT={mcts_total/total*100:.0f}%" if total > 0 else ""))
"""Compare V1 as P2 vs V2 as P1 — seat reversal test."""
...
def play_game(v1_as_p2=False):
    """V1 as P2 (if v1_as_p2) or V1 as P1."""
    ...
    for turn in range(56):
        ...
        t0 = time.time()
        if (mark == 1 and not v1_as_p2) or (mark == 2 and v1_as_p2):
            m = V1(board, mark, legal, 8, move_deadline=1.5)
        else:
            m = V2(board, mark, legal, 8, move_deadline=1.5)
...
if __name__ == '__main__':
    print("=== 8x7/5 Seat Reversal: V1 as P2 vs V2 as P1 ===")
    print(f"V1 (P2): full eval, depth 8")
    print(f"V2 (P1): simple eval, depth 10")
    print()

    v1_wins_p2 = 0
    v2_wins_p2 = 0  # V2 as P1 = V2 as P2 perspective
    draws = 0
    total_moves = 0
    total_time = 0.0
    n_games = 5
    ...
    for i in range(n_games):
        result, n_moves, gt, moves = play_game(v1_as_p2=True)
        total_moves += n_moves
        total_time += gt

        if 'P1_WINS' in result:
            v2_wins_p2 += 1  # P1 is V2
            print(f"Game {i+1}: V2(P1) wins | {n_moves} moves | {gt:.1f}s")
        elif 'P2_WINS' in result:
            v1_wins_p2 += 1  # P2 is V1
            print(f"Game {i+1}: V1(P2) wins | {n_moves} moves | {gt:.1f}s")
        else:
            draws += 1
    ...
    print(f"V1 as P2: {v1_wins_p2}W")
    print(f"V2 as P1: {v2_wins_p2}W")
    print(f"Draws: {draws}")

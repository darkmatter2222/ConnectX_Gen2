"""Tests for the 8x7/5 tactical MCTS bot."""
...
def test_import():
    ...
    assert callable(mcts_tactical_bot_8x7_5)
    assert callable(mcts_tactical_bot_fast_8x7_5)


def test_from_package():
    from connectx.bots import mcts_tactical_bot_8x7_5, mcts_tactical_bot_fast_8x7_5
    assert callable(mcts_tactical_bot_8x7_5)
    assert callable(mcts_tactical_bot_fast_8x7_5)
...
def test_empty_board():
    ...
    m = mcts_tactical_bot_fast_8x7_5(board, 1, legal, 8)
    assert m in legal
...
def test_legal_moves():
    ...
    for turn in range(16):
        ...
        if not legal:
            break
        m = mcts_tactical_bot_fast_8x7_5(board, mark, legal, 8)
...
def test_timing():
    ...
    legal = connectx.valid_moves(board, 8)
    t0 = time.time()
    m = mcts_tactical_bot_fast_8x7_5(board, 1, legal, 8, move_deadline=1.5)
    elapsed = time.time() - t0
    assert elapsed < 5.0, f"Tactical MCTS took {elapsed:.3f}s (expected < 5s)"
    assert m in legal
...
def test_timing_after_pieces():
    ...
    m = mcts_tactical_bot_fast_8x7_5(board, 1, legal, 8, move_deadline=1.5)
    elapsed = time.time() - t0
    assert elapsed < 3.0, f"Tactical MCTS took {elapsed:.1f}s with pieces"
    assert m in legal
...
def test_full_depth():
    board = connectx.make_board(7, 8)
    legal = connectx.valid_moves(board, 8)
    m = mcts_tactical_bot_8x7_5(board, 1, legal, 8)

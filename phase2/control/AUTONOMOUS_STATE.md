...
**Session:** Cycle 34
**Date:** 2026-08-08
**Status:** Cycle 34: P2-exploit bot fully tested (11/11 tests pass). Benchmark shows all-draws vs v2 (40/40 draws), confirming both bots play near-optimally. P2 bottleneck persists but P2 bot is competitive. v0009 built and validated. Next: build hybrid AB+MCTS P2 strategy or explore 8×7/6.
...
- **Tactical playouts:** Win > block > center (85%) > random
- **Max iterations:** 2500 (fast), configurable (default)
- **PUCT constant:** C = 1.2
- **Mark-aware back-propagation:** Correctly attributes rewards from opponent perspective
- **5 new tests added** (35 total tests, all pass)

### Full Comparison: AB vs MCTS(UCB1) vs PUCT (60 games)

| Matchup | Result |
|---------|--------|
| MCTS(500) as P1 vs AB as P2 | AB wins 10/10 (54 moves) |
| AB as P1 vs MCTS(500) as P2 | **10 draws** (56 moves) |
| PUCT(2500) as P1 vs AB as P2 | **10 draws** (56 moves) |
| AB as P1 vs PUCT(2500) as P2 | **AB wins 10/10 (33 moves)** |
| PUCT(2500) as P1 vs MCTS(500) as P2 | **PUCT wins 10/10** (48 moves) |
| MCTS(500) as P1 vs PUCT(2500) as P2 | **10 draws** (56 moves) |

### Key Findings

1. **AB as P1 = absolute advantage** against both MCTS and PUCT — wins 10/10 either way
2. **PUCT as P2 loses FASTER** than UCB1 MCTS (33 vs 54 moves)
3. **PUCT as P1 draws same as UCB1 MCTS** (10/10 draws)
4. **PUCT does not outperform UCB1 MCTS** when PUCT is P2 and MCTS is P1 (MCTS draws 10/10)
5. **Deeper PUCT tree converges on AB's forced-win lines more efficiently** — counterintuitive

### Conclusion: PUCT selection + tactical playouts do NOT improve MCTS vs AB

The bottleneck is search paradigm, not exploration strategy:
- AB with depth-8 bitboard search solves millions of positions/move
- MCTS explores thousands with full board copies
- Tactical playouts are good but insufficient to find AB's deeper forced lines

### Next Steps
1. **Build 8×7/5 opening book** — pre-compute AB's early-game optimal moves
...
**Two deliverables this cycle: opening book improvement and new MCTS variant.**

### Opening Book v2: 3,069 entries (13× improvement)

- **Original book:** 237 entries, built with branching=5, depth=8, 120s timeout
- **New book:** 3,069 entries, built with branching=4, depth=7, 600s timeout
- **Coverage:** 3,069 unique board positions, 6,138 total entries (both marks)
- **Hit rate:** 40% at depth 1, 23% at depth 2, 16% at depth 3, 11% at depth 4
- **Book size:** ~295 KB JSON
- **Booked bot:** Uses same API (`bitboard_ab_bot_fast_8x7_5_booked`), drops to AB search for book misses

### Tactical MCTS Bot: `mcts_tactical_bot_8x7_5`

- **Enhanced playouts** over vanilla MCTS PUCT:
  1. **Win detection:** Immediate win search (same as PUCT)
  2. **Block detection:** Block opponent's immediate win (same as PUCT)
  3. **Threat creation:** Create 4-in-a-line threats (new — fork-seeking)
  4. **Threat blocking:** Block opponent's threats proactively (new)
  5. **Heuristic eval:** At terminal positions, score center control + adjacency + height (new)
- **Algorithm:** PUCT selection + tactical playouts + heuristic terminal evaluation
- **Fast variant:** 2,500 iterations (mcts_tactical_bot_fast_8x7_5)
- **Full variant:** Configurable iterations (mcts_tactical_bot_8x7_5)
- **Performance:** 0.19-0.31s per move on empty board, <0.1s with pieces
...
| `book_8x7_5.json` | Updated to 3,069 entries |
...
2. **Tactical MCTS is fundamentally different** — instead of just looking for wins/blocks,
   it creates threats and blocks opponent threats during playouts. This gives MCTS a much
   more aggressive style that could lead to better playout quality.
3. **Tactical MCTS is ~0.25s per move** — comparable to standard PUCT MCTS. The threat

...
| TonyCWang dataset as supervised pre-training source | 11 | 958M rows of exact optimal evaluations from Pascal Pons solver; board-state input (2×6×7) maps to ResNet; faster convergence than AlphaZero self-play | Provides ground-truth training data; supervised pre-training approach added to architecture rankings | AlphaZero-style self-play (longer convergence) | Yes — GPU training compute required |
| Pascal Pons solver as value oracle | 11 | C++ negamax+PVS+TT+book with iterative binary search; generates exact game values for training data | Provides perfect-play evaluation for Connect 4; solver source fully decoded | Using a less-optimized solver (BitBully-style) | Yes — solver choice affects training data quality |
| blog.gamesolver.org tutorial unreachable | 11 | SSL certificate mismatch — served GitHub cert instead of proper gamesolver.org cert; cannot verify tutorial content | Pascal Pons solver tutorial cannot be verified; alternative documentation sources needed | blog.gamesolver.org is reachable | No — domain misconfigured |
| GitHub API unreachable | 11 | api.github.com returns TLS/schannel certificate errors; no curl or WebFetch output possible | GitHub topic discovery and source code fetching via API now blocked; must use WebFetch-only approach | GitHub API is working | Yes — if network issue resolves |
| LLM-based Connect 4 approach is inferior to board-state | 11 | Text-based autoregressive prediction (11+ Hugging Face models) lacks evaluation metrics; compounding error from sequential move prediction; board-state (TonyCWang) is theoretically superior | Supervised pre-training with board-state inputs is the preferred approach; text-based models are not competitive without evaluation evidence | LLM text prediction as primary approach | Yes — different architecture choice |
| Evidence audit necessary | 11 | 17 structural issues found in corpus: duplicate sections, stale headers, duplicate sources, dead source citations | Corpus quality improved by removing duplicates and updating metadata; no substantive claim changes | No audit — proceed without cleanup | Yes — audit is one-time cleanup |
| Kaggle kaggle-environments config is backward-compatible | 13 | episodeSteps/runTimeout moved to global schemas.json; agentTimeout removed; remainingOverageTime moved to observation; game rules unchanged | Functional behavior unchanged; structural refactoring of config system; all prior Kaggle claims remain valid | Config structure may change further | Yes — if behavioral changes occur |

...
| S066 | tromp/fhourstones88 — 8x8 Connect Four solver with book88 solving book (binary database of solved positions up to 16 plies) — Round 16 | VERIFIED + source code — Round 16 | Binary file format: bitboard + Result (score: short, work: int, bestmove: int) stored via read(bd, &bb, BBYTES) then read(bd, &rslt, sizeof(short)). Loaded into BookMap hash table. C488 binary accepts positions via stdin, solves instantly if in book, searches if not. Positions with work >= BOOKWORK (24) dynamically added to book. ~500MB memory default (TRANSIZE=8306069). Available as downloadable binary file in GitHub repo. |
| S067 | Pascal Pons/connect4 — OpeningBook.hpp (C++ header) + generator.cpp (book generation) — Round 16 | VERIFIED + source code — Round 16 | AGPL v3 license. OpeningBook.hpp provides opening book logic. generator.cpp generates the book. DEPTH=14 configuration for 7x6 board. Supports configurable board sizes via template WIDTH/HEIGHT parameters (up to 9x6 in uint64_t). Book generated at build time from source code — no binary book file distributed in repo. |
| S068 | tristan852/kite — Java Connect 4 solver with 15-ply opening book (Maven Central, JITPack, CLI binaries) — Round 16 | VERIFIED + source code — Round 16 | GPL-3.0 license. Java 25+ with GraalVM native image CLI. Precomputed compiled cache file (opening.cfc, 95.6 MB) loaded from resources. Alpha-beta pruning + symmetry reduction + bitboards + position hashing + transposition caching. 13 skill levels (0-2000 Elo). Outperforms C++ (Fhourstones, Pascal Pons) and Rust (Ben Rall) solvers on Pascal Pons benchmark (6,000 positions across 6 categories). Open-source Java library available on Maven Central. |
| S069 | tristan852/kite OpeningBoardScoreCache.java — 6-bit packed score cache with mixed hash scheme (32 MiB bucket seeds, 58.9 MiB packed scores) — Round 16 | VERIFIED + source code — Round 16 | Custom binary cache format: bucketSeeds (33,554,432 bytes = 2^25 entries, each 1 byte) + packedBoardScores (61,737,771 bytes). 6-bit packed scores (range -18 to +39, offset by -18). Three-key mixed hash (0x9E3779B97F4A7C15L, 0xBF58476D1CE4E5B9L, 0x94D049BB133111EBL). Lookup: bucketIndex = mixedHash & 0x1FFFFFFF, bucketSeed = bucketSeeds[bucketIndex], index = mixLong(mixedHash, bucketSeed) % 82317028, read 6 bits at bit position. Loading: Java reads from resource bundle or external bytes. Used at filledCellAmount <= 15 (OPENING_SCORE_CACHE_MAXIMAL_DEPTH = 15). |
...
| S070 | tre-systems/rowspire — worker/src/evaluation.rs: evaluate_with_genetic_params() using evolved weights | raw.githubusercontent.com/tre-systems/rowspire/main/worker/src/evaluation.rs | Source code | ~2025 | Full evaluation function: positional_score (column centering + row height) + weighted_feature_score (7 features × evolved weights). Defensive move detection: counts opponent's winning moves if they play (5000 per move). Win detection at top: +10000/-10000 (or evolved win_score/loss_score). |
| S071 | tre-systems/rowspire — worker/src/ evaluation_lines.rs: Shared threat line evaluation primitives | raw.githubusercontent.com/tre-systems/rowspire/main/worker/src/evaluation_lines.rs | Source code | ~2025 | line_threat() with extended scoring: 4-in=1000, 3-unblocked=100, 3-blocked=10, 2-unblocked=10, 2-blocked=1, 1-unblocked=1, else 0. consecutive_score() for vertical/horizontal alignment counting. |

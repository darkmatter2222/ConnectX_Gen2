...
## Sources Added Round 30 (EXTERNAL-POOL BATCH-00016)
...
## R48 New Sources (S174–S183) — NN-005 Model Compression Sources

| Source ID | Title | URL / Path | Type | Date | Notes |
|-----------|-------|------------|------|------|-------|
| S174 | Han et al. (2015) "Deep Compression: Compressing Deep Neural Networks with Pruning, Trained Quantization and Huffman Coding" | https://arxiv.org/abs/1510.00149 | Academic paper | 2015 | Foundational deep compression paper. Pruning + quantization + Huffman coding pipeline. GPU/CPU deployment benchmarks. |
| S175 | Hinton et al. (2015) "Distilling the Knowledge in a Neural Network" | https://arxiv.org/abs/1503.02531 | Academic paper | 2015 | Knowledge distillation origin paper. Temperature scaling, soft target training. CNN compression from VGG to small network. |
| S176 | Frankle & Carbin (2018) "The Lottery Ticket Hypothesis" | https://arxiv.org/abs/1803.03635 | Academic paper | 2018 | Pruning hypothesis: sparse subnetworks findable at random initialization can train to target accuracy. 30-50x compression. |
| S177 | Google TensorRT documentation — INT8 quantization and conversion | https://docs.nvidia.com/deeplearning/tensorrt/developer-guide/index.html | Documentation | Latest | TensorRT INT8 quantization API, calibration, INT8 precision. Latency benchmarks. |
| S178 | ONNX Runtime quantization documentation — PTQ and QAT | https://onnxruntime.ai/docs/performance/model-optimization/ | Documentation | Latest | ONNX Runtime PTQ/QAT for INT8, ONNX optimization passes. CPU and GPU deployment. |
| S179 | Stockfish NNUE quantization methodology (int32_t QA=127) | github.com/official-stockfish/Stockfish/blob/master/src/nnue_nnue.h | Source code | Latest | NNUE accumulator quantization: int32_t accumulator, QA=127 (7-bit), precise scaling with rounding. Critical reference for game AI quantization. |
| S180 | PyTorch pruning API documentation | https://pytorch.org/docs/stable/pruning.html | Documentation | Latest | prune.l1_unstructured, prune.global_unstructured, prune.ln_structured. Manual prune vs persistent pruning. |
| S181 | PyTorch quantization documentation (PTQ, QAT) | https://pytorch.org/docs/stable/quantization.html | Documentation | Latest | torch.quantization.quantize_dynamic, torch.quantization.quantize_per_tensor, PTQ flow, QAT flow. |
| S182 | Mocaru et al. (2018) "Scalable Learning of Non-Transitive Symmetric Games" (PNAS) — game-playing neural networks | https://www.pnas.org/doi/10.1073/pnas.1720172115 | Academic paper | 2018 | PNAS paper on game-playing NNs. Self-play training, symmetry exploitation, board-size generalization. |

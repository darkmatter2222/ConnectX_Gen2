"""Quick v2 vs v3 head-to-head with 2s time limits."""
import sys
sys.path.insert(0, '.')
import time
...
def matchup(bot_a_fn, bot_b_fn, name_a, name_b, games=10, time_limit=2.0):
    ...
    print(f'{name_a} vs {name_b}: {name_a}={a_w}/{total} ({a_w/total*100:.0f}%) '
          f'  avg_move_time: {name_a}={avg_a*1000:.0f}ms {name_b}={avg_b*1000:.0f}ms')
    return a_w, b_w, draws


if __name__ == '__main__':
    print("=== v3 vs v2 (10 games) ===")
    matchup(bitboard_ab_bot_v3, bitboard_ab_bot_v2, 'v3', 'v2', 10)

    print("\n=== v3 vs wsb (10 games) ===")
    matchup(bitboard_ab_bot_v3, win_seek_block_bot, 'v3', 'wsb', 10)

    print("\n=== v3 vs mcts (10 games) ===")
    matchup(bitboard_ab_bot_v3, mcts_bot, 'v3', 'mcts', 10)

    print("\n=== v3_fast vs v2_fast (10 games) ===")
    matchup(bitboard_ab_bot_fast_v3, bitboard_ab_bot_fast_v2, 'v3f', 'v2f', 10)

    print("\n=== v3_fast vs wsb (10 games) ===")
    matchup(bitboard_ab_bot_fast_v3, win_seek_block_bot, 'v3f', 'wsb', 10)

    print("\n=== v3_fast vs mcts (10 games) ===")
    matchup(bitboard_ab_bot_fast_v3, mcts_bot, 'v3f', 'mcts', 10)

    print("\n=== Baseline: v2 vs wsb (10 games) ===")
    matchup(bitboard_ab_bot_v2, win_seek_block_bot, 'v2', 'wsb', 10)
